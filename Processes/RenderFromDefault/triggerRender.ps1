class ProcessSettings {
    ...
    [array]GenerateProcessInformation(){
        $argumentArray = ('-b', '--factory-startup', '-o', $this.outputFilepathWithHashes, '-P', $this.absoluteScriptLocation, '-F', "PNG", '-f', '1', '--', '-file', (Join-Path -Path $this.fileLocation -ChildPath $this.filename), '-settings', $this.absoluteRenderSettings)
